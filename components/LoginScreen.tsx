  ...
  return (
    ...
    >
      ...
      <div className="w-full max-w-sm space-y-3">
        ...
        {step === 'landing' && (
          <>
            {/* Sign in with Apple — App Store guideline 4.8 requires this to be
                offered as an equivalent option (not buried) when Google login
                is present. Keep it first/prominent. */}
            <button
              onClick={() => signIn('apple', { callbackUrl: '/' })}
              className="w-full flex items-center gap-3 px-4 py-3.5 rounded-2xl bg-black text-white text-sm font-semibold hover:bg-slate-900 transition-all"
              style={{ boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}
            >
              <Apple size={18} />
              <span>{lang === 'zh' ? '以 Apple 繼續' : 'Continue with Apple'}</span>
            </button>
